...
function buildHome() {
  ...
  const faqs = [
    ...
    { q: "How much does a garage door repair cost in Greater Vancouver?", a: `Most common repairs are flat-priced (tap Pricing in the footer for spring numbers): single spring ${px("(tap Pricing)", "$" + cfg.springPricing.singleSpring.price)}, two springs with free cables ${px("(tap Pricing)", "$" + cfg.springPricing.twoSpringsNewCables.price)}, cable repair from $${cfg.otherPricing.cableRepairFrom}, opener repair from $${cfg.otherPricing.openerRepairFrom}. You always get a written quote before any work — no $19.99 bait, no surprises.` },
...
function buildFaq() {
  ...
  const faqs = [
    ...
    { q: "How much does garage door repair cost in Greater Vancouver?", a: `Most repairs are flat-priced (tap Pricing in the footer for the spring numbers): single spring ${px("(tap Pricing)", "$" + cfg.springPricing.singleSpring.price)}, two springs with free cables ${px("(tap Pricing)", "$" + cfg.springPricing.twoSpringsNewCables.price)}, two high-cycle springs ${px("(tap Pricing)", "$" + Number(cfg.springPricing.twoSpringsHighCycle.price).toLocaleString())}, cable repair from $${cfg.otherPricing.cableRepairFrom}, off-track from $${cfg.otherPricing.offTrackFrom}, opener repair from $${cfg.otherPricing.openerRepairFrom}, tune-up from $${cfg.otherPricing.tuneUpFrom}. You always get a written quote first.` },
